...
class Path:
    ...
    def finished(self):
        if self.path[-1] == (MAXX, MAXY) and self.same_dir >= 4:
            return True
        return False
    ...
    def move(self):
        ...
        if 0 <= x <= MAXX and 0 <= y <= MAXY:
            ...
            if key not in visited or visited[key] > self.cost:
                if self.same_dir < 4:
                    visited[key] = self.cost
                else:
                    for k in range(self.same_dir, 10):
...
stack = []
stack.append(Path(0, 0, "R"))
stack.append(Path(0, 0, "D"))
...
while stack:
    ...
    if path.finished():
        if minimum is None or path.cost < minimum:
            minimum = path.cost
        print(path.cost, "         ", len(stack), 100*len(visited)/(MAXX*MAXY*4*6))
        continue
